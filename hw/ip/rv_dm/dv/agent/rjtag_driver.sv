...
class rjtag_driver extends uvm_driver#(rjtag_seq_item);
  ...
  task jtag_shift_ir(int alen, bit [4:0] addr);
    ...
    vif.tms = 1'b1;
    vif.tdi = 1'b0;  // SelectDR
    @(negedge vif.tck);
    vif.tms = 1'b1;
    vif.tdi = 1'b0;  // SelectIR
    @(negedge vif.tck);
    vif.tms = 1'b0;
    vif.tdi = 1'b0;  // CaptureIR
    @(negedge vif.tck);
    vif.tms = 1'b0;
    vif.tdi = 1'b0;  // ShiftIR
  ...
  task jtag_shift_dr(int dlen, logic [63:0] data, logic [63:0] dout);
    ...
    vif.tms = 1'b1;
    vif.tdi = 1'b0;  // go to SelectDR
    @(negedge vif.tck);
    vif.tms = 1'b0;
    vif.tdi = 1'b0;  // go to CaptureDR
    @(negedge vif.tck);
    vif.tms = 1'b0;
    vif.tdi = 1'b0;  // go to ShiftDR
    for (int i = 0; i < dlen - 1; i++) begin
      @(negedge vif.tck);
      vif.tms = 1'b0;
      vif.tdi = data[i];  // stay in ShiftDR
      @(posedge vif.tck);
      dout[i] = vif.tdo;
    end
    @(negedge vif.tck);
    vif.tms = 1'b1;
    vif.tdi = data[dlen-1];  // go to Exit1DR
    @(posedge vif.tck);
    dout[dlen-1] = vif.tdo;
    @(negedge vif.tck);
    vif.tms = 1'b1;
    vif.tdi = 1'b0;  // go to UpdateIR
    @(negedge vif.tck);
    vif.tms = 1'b0;
    vif.tdi = 1'b0;  // go to RTI
